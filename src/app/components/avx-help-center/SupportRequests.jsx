        ...
        <section className="relative py-10 px-4 sm:px-6 overflow-hidden font-secondary text-primary flex flex-col justify-center">
            ...
            <div className="max-w-7xl mx-auto w-full relative">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 sm:mb-10">
                    <div>
                        <p className="text-sm tracking-[0.4em] uppercase text-third/60 font-semibold mb-1 font-primary">Support Center</p>
                ...
                {/* Panel */}
                <div className="relative border border-primary/10 rounded-2xl sm:rounded-3xl overflow-hidden backdrop-blur-sm ">
                    ...
                    {/* Table header */}
                    <div className="hidden sm:grid px-6 sm:px-8 py-3 border-b border-primary/5" style={{ gridTemplateColumns: "140px 1fr 150px 130px 44px" }}>
                        {["Ticket ID", "Subject / Category", "Status", "Last Updated", ""].map((h, i) => (
                            <span key={i} className="text-[12px] uppercase tracking-[0.3em] font-bold text-third/30 font-primary">{h}</span>
                        ))}
                    </div>
                                ...
                                <div key={ticket.id}>
                                    <div
                                        className={`relative px-6 sm:px-8 py-4 sm:py-5 cursor-pointer transition-all duration-200 ${
                                            isActive ? "bg-fourth/5" : isHovered ? "bg-primary/5" : "bg-transparent"
                                        }`}
                                    ...
                                    >
                                        ...
                                        <div className="hidden sm:grid items-center gap-4" style={{ gridTemplateColumns: "140px 1fr 150px 130px 44px" }}>
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-xs text-third/35 mt-0.5 uppercase tracking-widest font-primary">{ticket.category}</p>
                                            </div>
                                            <div>
                                            ...
                                            <span className="text-xs text-third/35 font-secondary">{ticket.updated}</span>
                                                ...
                                                {ticket.messages.map((msg, mi) => (
                                                    <div key={mi} className={`flex flex-col gap-1 ${msg.mine ? "items-end" : "items-start"}`}>
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-xs font-bold text-third/40 uppercase tracking-wider font-primary">{msg.from}</span>
                    ...
                    <div className="px-6 sm:px-8 py-4 flex items-center justify-between border-t border-primary/5">
                        ...
                        <button className="w-7 h-7 rounded-lg text-xs font-bold bg-fourth/15 text-fourth border border-fourth/25 font-primary">1</button>
                    </div>
                </div>
            </div>
        </section>
